	...
	// so that the passes can register themselves to the graph
	{
		...
		if (AllowFullPostProcessing(View, FeatureLevel))
		{
			...
			if( AntiAliasingMethod == AAM_TemporalAA && ViewState)
			{
				...
				// FB Bulgakov Begin - Ghosting fix
				{
					TAAParameters.MiscParameters = FVector4(1.0f, 40.0f, 0.0f, 0.0f);

					static IConsoleVariable* CVarTAAHighSpeedBlendFactor = IConsoleManager::Get().FindConsoleVariable(TEXT("r.TemporalAA.HighSpeedBlendFactor"));
					if (CVarTAAHighSpeedBlendFactor)
						TAAParameters.MiscParameters.X = CVarTAAHighSpeedBlendFactor->GetFloat();

					static IConsoleVariable* CVarTAAMaxSpeed = IConsoleManager::Get().FindConsoleVariable(TEXT("r.TemporalAA.MaxSpeed"));
					if (CVarTAAMaxSpeed)
						TAAParameters.MiscParameters.Y = CVarTAAMaxSpeed->GetFloat();
				}
				// FB Bulgakov End
