...
class ENGINE_API URendererSettings : public UDeveloperSettings
{
	...
	UPROPERTY(config, EditAnywhere, Category = DefaultSettings, meta = (
		EditCondition = "DefaultFeatureAntiAliasing == AAM_TemporalAA",
		ConsoleVariable = "r.TemporalAA.HighSpeedBlendFactor", DisplayName = "Temporal High Speed Blend Factor",
		ToolTip = "Blend Factor used for pixels with high screen space speed"))
	float TAAHighSpeedBlendFactor = 1.0f;

	UPROPERTY(config, EditAnywhere, Category = DefaultSettings, meta = (
		EditCondition = "DefaultFeatureAntiAliasing == AAM_TemporalAA",
		ConsoleVariable = "r.TemporalAA.MaxSpeed", DisplayName = "Temporal High Speed",
		ToolTip = "Max Speed when TAA uses TAAHighSpeedBlendFactor"))
	float TAAMaxSpeed = 40.0f;
	// FB Bulgakov End
